// Supabase and data functions extracted from index.html
...
// Helper functions for timeout and queue operations
...
const queueOperation = (operationKey, operation) => {
    if (operationQueue.has(operationKey)) {
        return operationQueue.get(operationKey);
    }
...
// Helper functions for connection status and notifications
...
// Main dataStore object with all database operations
const dataStore = {
    async getTrades() {
        let retryCount = 0;
        const maxRetries = 3;
        ...
        try {
            if (supabase) {
                const { data, error } = await withTimeout(
                    supabase.from('challenges').select('*').eq('user_id', window.appState?.user?.id).order('createdAt', { ascending: false }),
    ...
    async getPartialExits(tradeId) {
        try {
            if (supabase && window.appState?.user?.id) {
                const { data, error } = await withTimeout(
                    supabase.from('partial_exits').select('*').eq('trade_id', tradeId).eq('user_id', window.appState.user.id).order('exit_date', { ascending: true }),
        ...
        try {
            if (supabase && window.appState?.user?.id) {
                const { error } = await withTimeout(
                    supabase.from('partial_exits').delete().eq('id', partialExitId).eq('user_id', window.appState.user.id),
    ...
    async deleteChallenge(challengeId) {
        try {
            if (supabase && window.appState?.user?.id) {
                const { error } = await withTimeout(
                    supabase.from('challenges').delete().eq('id', challengeId).eq('user_id', window.appState.user.id),
...
// Export auth and dataStore to window
window.auth = auth;
